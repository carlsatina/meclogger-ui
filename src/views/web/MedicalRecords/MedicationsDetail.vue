...
<div class="detail-shell medical-shell">
    ...
    <div class="detail-wrapper">
        <div class="detail-header">
            ...
            <div>
                <h2 class="detail-title">Medications</h2>
        ...
        <template v-else>
            <div v-if="medications.length" class="med-grid">
                <div class="med-card glass-card" v-for="med in medications" :key="med.id">
                    <div class="med-card-header">
                        ...
                        <div class="med-info">
                            ...
                            <p v-if="med.dosage" class="med-meta">{{ med.dosage }}</p>
                        </div>
                    ...
                    <p v-if="med.instructions && editingId !== med.id" class="med-instructions">{{ med.instructions }}</p>

                    <div v-if="editingId === med.id" class="edit-form">
                        ...
                        <div class="edit-actions">
                            <button class="dose-btn took" :disabled="savingId === med.id" @click="saveEdit(med)">
                                <mdicon name="check" :size="15"/> Save
                            </button>
                            <button class="dose-btn skip" @click="cancelEdit()">Cancel</button>
                        </div>
                    </div>

                    <div v-if="editingId !== med.id" class="med-stats-row">
                        <span class="last-taken" :class="{ muted: !lastTaken(med) }">
                            <mdicon :name="lastTaken(med) ? 'check-circle-outline' : 'clock-outline'" :size="14"/>
                            {{ lastTaken(med) ? 'Last taken ' + formatRelative(lastTaken(med)) : 'No doses logged yet' }}
                        </span>
                        <span class="adherence-label">30-day adherence</span>
                    </div>

                    <div v-if="editingId !== med.id" class="med-footer">
                        <span v-if="med.startDate" class="med-date">Started {{ formatDate(med.startDate) }}</span>
                        <div class="med-actions">
                            ...
                            <button class="dose-btn remind" @click="setReminder(med)" title="Set reminder">
                                <mdicon name="bell-outline" :size="15"/>
                            </button>
                            <button class="dose-btn edit" @click="startEdit(med)" title="Edit">
                            ...
                            >
                                <mdicon name="trash-can-outline" :size="15"/>
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div v-else class="state-box empty">
                ...
                <p class="empty-title">No medications saved yet</p>
...
.med-card-header { display: flex; align-items: flex-start; gap: 12px; }
...
.med-info { flex: 1; }
.med-name { font-size: 15px; font-weight: 700; color: var(--text-primary); margin: 0 0 2px; }
.med-meta { font-size: 13px; color: var(--text-secondary); margin: 0; }
...
.med-instructions { font-size: 13px; color: var(--text-muted); font-style: italic; margin: 0; }
...
.adherence-label { font-size: 12px; color: var(--text-muted); }
...
.med-date { font-size: 12px; color: var(--text-muted); }
...
.dose-btn:hover { opacity: 0.85; }
.dose-btn:active { transform: scale(0.97); }
.dose-btn:disabled { opacity: 0.4; cursor: not-allowed; }
