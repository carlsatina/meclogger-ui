...
<div class="logbook-page">
    ...
    <section class="logbook-section">
        ...
        <div v-if="showGroupA" class="glass-card table-card">
                    ...
                    <thead>
                        <tr>
                            <th class="sortable" @click="toggleSort('A', 'date')" :aria-sort="ariaSort(sortA, 'date')">
                                Date
                                <mdicon v-if="sortA.key === 'date'" :name="sortA.dir === 'asc' ? 'chevron-up' : 'chevron-down'" size="14" class="sort-icon"/>
                            </th>
                            <th v-if="showMain" class="sortable" @click="toggleSort('A', 'main')" :aria-sort="ariaSort(sortA, 'main')">
                                Main Category
                                <mdicon v-if="sortA.key === 'main'" :name="sortA.dir === 'asc' ? 'chevron-up' : 'chevron-down'" size="14" class="sort-icon"/>
                            </th>
                            <th v-if="showSub" class="sortable" @click="toggleSort('A', 'sub')" :aria-sort="ariaSort(sortA, 'sub')">
                                Sub Category
                                <mdicon v-if="sortA.key === 'sub'" :name="sortA.dir === 'asc' ? 'chevron-up' : 'chevron-down'" size="14" class="sort-icon"/>
                            </th>
                            <th class="sortable" @click="toggleSort('A', 'description')" :aria-sort="ariaSort(sortA, 'description')">
                                Description
                                <mdicon v-if="sortA.key === 'description'" :name="sortA.dir === 'asc' ? 'chevron-up' : 'chevron-down'" size="14" class="sort-icon"/>
                            </th>
                            <th class="number sortable" @click="toggleSort('A', 'amount')" :aria-sort="ariaSort(sortA, 'amount')">
                                Amount
                                <mdicon v-if="sortA.key === 'amount'" :name="sortA.dir === 'asc' ? 'chevron-up' : 'chevron-down'" size="14" class="sort-icon"/>
                            </th>
                            <th class="number sortable" @click="toggleSort('A', 'balance')" :aria-sort="ariaSort(sortA, 'balance')">
                                Balance
                                <mdicon v-if="sortA.key === 'balance'" :name="sortA.dir === 'asc' ? 'chevron-up' : 'chevron-down'" size="14" class="sort-icon"/>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
    ...
    <section class="logbook-section">
        ...
        <div v-if="showGroupB" class="glass-card table-card">
            <div v-if="loading" class="empty-state">Loading history…</div>
            <div v-else-if="error" class="empty-state error-text">{{ error }}</div>
            <div v-else class="table-wrap">
                <table class="logbook-table">
                    <thead>
                        <tr>
                            <th class="sortable" @click="toggleSort('B', 'date')" :aria-sort="ariaSort(sortB, 'date')">
                                Date
                                <mdicon v-if="sortB.key === 'date'" :name="sortB.dir === 'asc' ? 'chevron-up' : 'chevron-down'" size="14" class="sort-icon"/>
                            </th>
                            <th v-if="showMain" class="sortable" @click="toggleSort('B', 'main')" :aria-sort="ariaSort(sortB, 'main')">
                                Main Category
                                <mdicon v-if="sortB.key === 'main'" :name="sortB.dir === 'asc' ? 'chevron-up' : 'chevron-down'" size="14" class="sort-icon"/>
                            </th>
                            <th v-if="showSub" class="sortable" @click="toggleSort('B', 'sub')" :aria-sort="ariaSort(sortB, 'sub')">
                                Sub Category
                                <mdicon v-if="sortB.key === 'sub'" :name="sortB.dir === 'asc' ? 'chevron-up' : 'chevron-down'" size="14" class="sort-icon"/>
                            </th>
                            <th class="sortable" @click="toggleSort('B', 'description')" :aria-sort="ariaSort(sortB, 'description')">
                                Description
                                <mdicon v-if="sortB.key === 'description'" :name="sortB.dir === 'asc' ? 'chevron-up' : 'chevron-down'" size="14" class="sort-icon"/>
                            </th>
                            <th class="number sortable" @click="toggleSort('B', 'amount')" :aria-sort="ariaSort(sortB, 'amount')">
                                Amount
                                <mdicon v-if="sortB.key === 'amount'" :name="sortB.dir === 'asc' ? 'chevron-up' : 'chevron-down'" size="14" class="sort-icon"/>
                            </th>
                            <th class="number sortable" @click="toggleSort('B', 'balance')" :aria-sort="ariaSort(sortB, 'balance')">
                                Balance
                                <mdicon v-if="sortB.key === 'balance'" :name="sortB.dir === 'asc' ? 'chevron-up' : 'chevron-down'" size="14" class="sort-icon"/>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
